...
use crate::models::{OptionalizedTask, Task};
use crate::responses::{
    errors::{
        InternalServerError,
        InvalidUuid,
...
fn all(connection: DbConnection) -> ApiResponse {
    return match tasks::table
        .order(tasks::columns::created_timestamp.desc())
...
fn read(id: String, connection: DbConnection) -> ApiResponse {
    ...
    return match tasks::table.find(id).get_result::<Task>(&*connection) {
        ...
        _ => NotFound(ResponseMessage::Custom(&format!(
            "No task found with id of {}",
            id
        ))),
    };
}
...
fn update(
    id: String,
    task: Option<Json<OptionalizedTask>>,
    ...
    let task = task.into_inner();
    let task = diesel::update(tasks::table.find(id))
        .set(&task)
        .get_result::<Task>(&*connection);
    return match task {
        Ok(task) => Success(&task),
...
fn delete(id: String, connection: DbConnection) -> ApiResponse {
    ...
    let task = diesel::delete(tasks::table.filter(tasks::columns::id.eq(id)))
        .get_result::<Task>(&*connection);
    return match task {
        Ok(task) => Success(&task),
        Err(e) => InternalServerError(ResponseMessage::Custom(&e.to_string())),
